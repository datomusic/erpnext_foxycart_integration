...
# This encrypted data was captured from a FoxyCart test transaction.
# It includes a customer, address, two items, and a transaction with type 'test_gateway'.
...
test_dependencies = ["Account", "Fiscal Year", "Mode of Payment"]
...
class TestProcessOrder(unittest.TestCase):
	def setUp(self):
		...
		self.setup_settings()
		self.decrypted_data = decrypt_data(ENCRYPTED_DATA, "test")

	def tearDown(self):
		frappe.db.rollback()

	def create_test_dependencies(self):
		if not frappe.db.exists("Mode of Payment", "Test Gateway"):
			frappe.get_doc({
		...
		if not frappe.db.exists("Account", "Debtors - FC"):
			frappe.get_doc({
				"doctype": "Account",
				"name": "Debtors - FC",
				"account_name": "Debtors - FC",
	...
	def setup_settings(self):
		self.settings = frappe.get_single("Foxycart Settings")
		self.settings.set("api_key", "test")
		self.settings.submit_sales_order = 0
		self.settings.submit_payment_entry = 0
		self.settings.payment_gateway_mappings = []
		self.settings.append("payment_gateway_mappings", {
			"foxycart_gateway_name": "test_gateway",
			"mode_of_payment": "Test Gateway",
			"payment_account": "Debtors - FC"
		})
		self.settings.save()

	def test_draft_documents_created(self):
		process_new_order(self.decrypted_data)

		# 1. Test Customer
		customer = frappe.get_doc("Customer", "Travis Ci")
		...
		self.assertEqual(payment_entry.docstatus, 0) # Draft
		self.assertEqual(payment_entry.party, "Travis Ci")
		self.assertEqual(payment_entry.paid_to, "Debtors - FC")
		self.assertEqual(payment_entry.references[0].reference_doctype, "Sales Order")
		self.assertEqual(payment_entry.references[0].reference_name, sales_order.name)

		# 4. Test Sales Invoice is not created
		self.assertFalse(frappe.db.exists("Sales Invoice", {"customer": "Travis Ci"}))

	def test_submitted_sales_order(self):
		self.settings.submit_sales_order = 1
		self.settings.save()
		
		process_new_order(self.decrypted_data)
		
		sales_order = frappe.get_last_doc("Sales Order")
		self.assertEqual(sales_order.docstatus, 1) # Submitted

		payment_entry = frappe.get_last_doc("Payment Entry")
		self.assertEqual(payment_entry.docstatus, 0) # Draft

	def test_submitted_payment_entry(self):
		self.settings.submit_payment_entry = 1
		self.settings.save()
	...
	def test_no_payment_mapping(self):
		self.settings.payment_gateway_mappings = []
		self.settings.save()

		process_new_order(self.decrypted_data)

		self.assertTrue(frappe.db.exists("Sales Order", {"customer": "Travis Ci"}))
		self.assertFalse(frappe.db.exists("Payment Entry", {"party": "Travis Ci"}))
